...
<main class="min-h-screen space-y-5 bg-neutral-900 p-5 text-sm text-neutral-200 select-none">
  {#if config}
    <section class="space-y-2">
      <h2 class="text-xs font-semibold tracking-wider text-neutral-500 uppercase">Monitor</h2>
      <select
        bind:value={config.monitor}
        class="w-full rounded-md border border-neutral-700 bg-neutral-800 px-2 py-1.5 outline-none focus:border-neutral-500"
      >
    ...
    <section class="flex gap-5">
      <div class="space-y-2">
        <h2 class="text-xs font-semibold tracking-wider text-neutral-500 uppercase">Dock</h2>
      ...
      <div class="flex-1 space-y-2">
        <h2 class="text-xs font-semibold tracking-wider text-neutral-500 uppercase">Offset</h2>
        {#each [['X', 'offsetX'], ['Y', 'offsetY']] as const as [label, key] (key)}
          <label class="flex items-center gap-2">
            <span class="w-3 text-neutral-400">{label}</span>
            <input
              type="number"
              bind:value={config[key]}
              class="w-full rounded-md border border-neutral-700 bg-neutral-800 px-2 py-1 font-mono outline-none focus:border-neutral-500"
            />
          </label>
        {/each}
      </div>
    ...
    <section class="space-y-3">
      <h2 class="text-xs font-semibold tracking-wider text-neutral-500 uppercase">Appearance</h2>
          ...
          <div class="flex items-center gap-1.5 text-xs text-neutral-400">
            ...
            {#if config[s.key] !== defaults[s.key]}
              <button
                ...
                class="text-neutral-500 transition-colors hover:text-emerald-300"
              >
                <ArrowCounterClockwiseIcon size="1.1em" weight="bold" />
              </button>
            {/if}
            <span class="font-mono text-neutral-300">{s.format(config[s.key])}</span>
          </div>
          <input
            type="range"
            aria-label={s.label}
      ...
      {/each}
      <label class="flex items-center justify-between" title="Tick on each bar marking how much of the window has elapsed">
        <span>Pace marker</span>
        <input type="checkbox" bind:checked={config.showPace} class="size-4 accent-emerald-400" />
      </label>
    </section>

    <section class="space-y-2">
      <h2 class="text-xs font-semibold tracking-wider text-neutral-500 uppercase">Providers</h2>
    ...
    <section class="flex items-center justify-between border-t border-neutral-800 pt-4">
      <label class="flex items-center gap-2">
        <input type="checkbox" bind:checked={autostart} onchange={toggleAutostart} class="size-4 accent-emerald-400" />
        <span>Start with Windows</span>
      </label>
      <button
        onclick={() => (config = { ...defaults, providers: {} })}
        class="text-xs text-neutral-500 hover:text-neutral-300">Reset</button
      >
    </section>
  {/if}
</main>
